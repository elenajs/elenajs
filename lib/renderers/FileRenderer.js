            ...
            try {
                dfs.stat(this.filename).then(
                        function(stat) {
                            try {
                                if (stat.isDirectory()) {
                                    self.handleError(self.request, self.response,
                                            self.deferred, 500,
                                ...
                                self.handleError(self.request, self.response, self.deferred, 500, err);
                            }
                        },
                        function(err) {
                            self.handleError(self.request, self.response, self.deferred, 404, err);
                        }
                );
            }
            catch (e1) {
