    ...
    return {
        ...
        normalize: function(/**string*/ id, /*Function*/ normalize) {
            ...
            if (id.charAt(0) === ".") {
                ...
                var referenceModuleDirname = require.toUrl(normalize("."));

                while (true) {
                    var newId = pathUtil.resolve(referenceModuleDirname, 'node_modules', id);
                    try {
                        fs.statSync(newId);
                        return newId;
                    } catch (e) {
                        var oldReferenceModuleDirname = referenceModuleDirname;
                        referenceModuleDirname = pathUtil.dirname(referenceModuleDirname);
                        if (referenceModuleDirname === oldReferenceModuleDirname) {
                            break;
                        }
                    }
                }
